...
public class Batalla {
	...
	public static void iniciarBatalla(Personaje personaje, Personaje enemigo) {
		...
		while (personaje.getVida() > 0 && enemigo.getVida() > 0) {
			if (turnoProtagonista) {
				...
				Batalla.ejecutarAtaque(true, personaje, enemigo);
				

			} else {

				System.out.println("Turno de " + enemigo.getNombre() + ":");
				Batalla.ejecutarAtaque(false,enemigo, personaje);
			}
	...
	public static int ejecutarAtaque(boolean esJugador, Personaje atacante, Personaje victima) {
		...
		int daño = atacante.getDanoAtaqueIndice(indiceAtaque);
